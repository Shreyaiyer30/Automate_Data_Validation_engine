...
#     st.metric("Data Quality Score", f"{analysis['quality_score']:.1f}/100")
...
#                 if rule_key == 'handle_missing': continue
                
#                 with rule_cols[i % 3]:
#                     if isinstance(rule_value, bool):
#                         enabled = st.toggle(rule_key.replace('_', ' ').title(), value=rule_value, key=f"{col}_{rule_key}")
